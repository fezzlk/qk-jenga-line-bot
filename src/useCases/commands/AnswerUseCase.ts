...
export class AnswerUseCase implements CommandUseCase {
  ...
  async execute({ groupId, userId, args }: CommandInput): Promise<string> {
    ...
    // Counters/participants are re-read and recomputed inside the transaction
    // (see AnswerFlowService.submitAnswer), so activeSession above is only used
    // to locate the session/round; its counter fields may already be stale here.
    const result = await this.answerFlow.submitAnswer(activeSession.id, round, userId, answerText);
