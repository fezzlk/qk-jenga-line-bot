...
describe("AnswerUseCase", () => {
  ...
  it("rejects a wrong answerer", async () => {
    const { useCase } = build(makeSession());

    const reply = await useCase.execute({ groupId: "g1", userId: "someone-else", args: "ab" });

    expect(reply).toContain("解答者ではありません");
  });
